<?php
	...
	$id_mois_requete = $con->query("SELECT id FROM mois WHERE annee='".$reporting_month."' AND mois='".$reporting_year."' AND is_validated=0 ")->fetch();
	...
	LEFT JOIN rapport r ON u.id=r.id_user  AND r.date_envoi>='".$date_du."' AND r.date_envoi<='".$date_au."'  WHERE u.id_profil=2";
		...
		if (isset($action) && $action == 'valider_rapport') 
		{
			...
			{
				$url="/admin_sans_validation/reporting_admin_SV?id_rapport=".$id_rapport;
				header("Location: $url");
			}
...
<!-- Search form -->
<form id="form" method="post" action="/admin_sans_validation/etat_envoi_SV">
	<table>
		<thead>
			<tr>
				<td>Annee</td>
...
<?php foreach ($data as $d): ?>
	...
	<td>
		...
		<form method="post" action="/admin_sans_validation/etat_envoi_SV">
